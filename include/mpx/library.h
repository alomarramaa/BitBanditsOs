...
typedef struct mcb {
    ...
    struct mcb *rel_prev; // Pointer to the previous MCB in the relative list (free/allocated)
    int is_free;          // Flag indicating whether the block is free or allocated
} MCB;

// Define heap manager data structure (2 lists)
typedef struct HeapManager{
    MCB *free_list;      // List 1: free memory blocks
    MCB *allocated_list; // List 2: allocated memory blocks
} HeapManager;
...
• Parameters: The total size of the heap (does not include the size of the initial MCB)
• Returns: None
*/
...
• Parameters: The size, in bytes, of the requested allocation
• Returns: NULL on error or a pointer to the start address of the newly allocated block (not the MCB
address)
*/
...
• Parameters: A pointer to the start address (not the MCB address) of an allocated block
